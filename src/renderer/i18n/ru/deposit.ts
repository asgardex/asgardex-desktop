...
const deposit: DepositMessages = {
  ...
  'deposit.interact.actions.checkMayaname': 'Проверить доступность',
  'deposit.interact.actions.addBondProvider': 'Добавить поставщика обеспечения',
  'deposit.interact.actions.getQuote': 'Get Quote',
  'deposit.interact.nodeFee.placeholder': 'Введите значение в %, мемо автоматически заполнится базисными пунктами',
  'deposit.interact.percentage.placeholder': 'Введите процент (0-100)',
  'deposit.interact.percentage.error': 'Процент должен быть от 0 до 100',
  'deposit.poolTransactionWarning': 'Отправить пул-транзакцию в {protocol}. Только для разработчиков — риск потери средств',
  'deposit.transferToken': 'Перевести токен {ticker}',
  ...
  'deposit.poolTransactionInfo':
    'Если включено, отправка осуществляется через контракт маршрутизатора {protocol} (depositWithExpiry), используемый для взаимодействия с пулом, например, для добавления ликвидности или бондинга. Оставьте ВЫКЛ для обычного перевода токенов.',

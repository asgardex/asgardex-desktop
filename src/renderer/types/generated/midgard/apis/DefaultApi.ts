...
export interface GetBorrowerDetailRequest {
    address: string;
}

export interface GetBorrowersAddressesRequest {
    asset?: string;
}
...
export interface GetSaversHistoryRequest {
    pool: string;
    interval?: GetSaversHistoryIntervalEnum;
    count?: number;
    to?: number;
    from?: number;
}
...
export class DefaultApi extends BaseAPI {
     ...
     * Actions List
     */
    getActions = ({ address, txid, asset, type, affiliate, limit, offset, nextPageToken, timestamp, height, prevPageToken, fromTimestamp, fromHeight }: GetActionsRequest): Observable<InlineResponse200> => {
        ...
        if (nextPageToken != null) { query['nextPageToken'] = nextPageToken; }
        if (timestamp != null) { query['timestamp'] = timestamp; }
        if (height != null) { query['height'] = height; }
        if (prevPageToken != null) { query['prevPageToken'] = prevPageToken; }
        if (fromTimestamp != null) { query['fromTimestamp'] = fromTimestamp; }
        if (fromHeight != null) { query['fromHeight'] = fromHeight; }
     ...
     * Borrower Details
     */
    getBorrowerDetail = ({ address }: GetBorrowerDetailRequest): Observable<BorrowerDetails> => {
        throwIfNullOrUndefined(address, 'getBorrowerDetail');

        return this.request<BorrowerDetails>({
            path: '/v2/borrower/{address}'.replace('{address}', encodeURI(address)),
            method: 'GET',
        });
    };
     ...
     * Borrowers List
     */
    getBorrowersAddresses = ({ asset }: GetBorrowersAddressesRequest): Observable<Array<string>> => {
     ...
     */
    getSaversHistory = ({ pool, interval, count, to, from }: GetSaversHistoryRequest): Observable<SaversHistory> => {
        throwIfNullOrUndefined(pool, 'getSaversHistory');
        ...
        return this.request<SaversHistory>({
            path: '/v2/history/savers/{pool}'.replace('{pool}', encodeURI(pool)),
            method: 'GET',
            query,
        });
    };

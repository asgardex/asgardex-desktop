...
import { SUIChain, SUIAsset } from '@xchainjs/xchain-sui'
...
export const createTransactionService = (client$: Client$, network$: Network$): TransactionService => {
    ...
    return FP.pipe(
            ...
            ({ msg }) =>
              Rx.of(
                RD.failure({
                  ...
                  msg: `Sending Ledger SUI tx failed. (${msg})`
                })
              ),
      ...
      RxOp.catchError((error) =>
        Rx.of(
          RD.failure({
            errorId: ErrorId.SEND_LEDGER_TX,
            msg: `Sending Ledger SUI tx failed. (${error?.message ?? String(error)})`
          })
        )
      ),
  ...
  // Vultisig transaction handler - placeholder for SUI
  const sendVultisigTx = (_params: { network: Network; params: SendTxParams }): TxHashLD =>
    Rx.of(RD.failure({ errorId: ErrorId.SEND_TX, msg: 'Vultisig not supported for SUI' }))

...
export const createTransactionService = (
  ...
  const runSendPoolTx$ = (client: AvaxClient, { ...params }: SendPoolTxParams, gasMultiplier: number): TxHashLD => {
    // helper for failures
  ...
  const runSendTx$ = (client: AvaxClient, params: SendTxParams, gasMultiplier: number): TxHashLD => {
    const failure$ = (msg: string) =>
      Rx.of<RD.RemoteData<ApiError, never>>(
        RD.failure({

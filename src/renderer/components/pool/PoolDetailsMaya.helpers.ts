...
import { PoolDetail, PoolStatsDetail } from '@xchainjs/xchain-mayamidgard'
...
export const getVolume24 = (data: Pick<PoolDetail, 'volume24h'>, priceRatio: BigNumber = bn(1)): AssetAmount =>
  baseToAsset(baseAmount(bnOrZero(data.volume24h).multipliedBy(priceRatio), CACAO_DECIMAL))
...
export const getVolumeTotal = (
  ...
  baseToAsset(
    baseAmount(bnOrZero(swapVolume), CACAO_DECIMAL)
...
export const getFees = (data: Pick<PoolStatsDetail, 'totalFees'>, priceRatio: BigNumber = bn(1)): AssetAmount =>
  baseToAsset(baseAmount(bnOrZero(data.totalFees).multipliedBy(priceRatio), CACAO_DECIMAL))

...
const transformList = ({ tokens }: Pick<ERC20Whitelist, 'tokens'>): AssetList =>
  FP.pipe(
    ...
    A.filterMap(({ address, symbol, logoURI }) =>
      FP.pipe(
        assetFromString(`${AVAXChain}.${symbol}-${address}`),
        O.fromNullable,
...
const createTemplate = (list: AssetList): string => {
  ...
  return `
    ...
    export const AVAX_TOKEN_WHITELIST: { asset: TokenAsset, iconUrl: O.Option<string> }[] = [${listAsString}]
  `
}
...
const writeList = (list: AssetList): TE.TaskEither<Error, void> =>
  FP.pipe(
    ...
    createTemplate,
    // "AVAX" _> AVAXChain
    S.replace(/"chain":"AVAX"/g, 'chain: AVAXChain'),
    (c) => writeFile(PATH, c)
  )
